...
import { PostPaginate, PostUrl, PostViewProps } from '@/app/types';
import { getTotalPages } from "@/utils/post"

export const getPostListPaths = async (): Promise<PostViewProps[]> => {
  ...
  for (let counter: number = 0; counter < totalPages; counter++) {
    ...
    if (nextPage > 1) {
      paths.push({ params: { slug: nextPage.toString() } })
    }
  }
  return paths
}
